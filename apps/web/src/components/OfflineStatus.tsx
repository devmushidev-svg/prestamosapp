...
export function OfflineStatus() {
  const { online, syncing, pending, attention, error, syncNow } = useOffline();
  const navigate = useNavigate();
  if (online && !syncing && pending === 0 && attention === 0 && !error) return null;
    ...
    : attention > 0
      ...
      : syncing
        ...
        : pending > 0
          ...
          : { label: "Datos preparados", Icon: CloudCheck, className: "border-pf-success/35 bg-pf-success-soft text-pf-success" };
  ...
  return (
    <button
      ...
      onClick={() => {
        if (attention > 0 || error) navigate("/ajustes");
        else void syncNow();
      }}
      disabled={syncing || (!online && attention === 0 && !error)}
