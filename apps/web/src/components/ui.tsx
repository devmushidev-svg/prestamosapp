...
export function Card({
  ...
  return (
    <div className={`pf-card-surface ${className}`} {...props}>
      {children}
    </div>
  );
}
...
export function Field({
  ...
  return (
    <div className={`block ${compact ? "space-y-0.5" : "space-y-1.5"} ${className}`}>
      <label
        htmlFor={htmlFor}
...
export function Modal({
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      const firstControl =
        dialogRef.current?.querySelector<HTMLElement>("[data-autofocus='true'], [autofocus]") ??
