import { CalendarClock, CheckCircle2, FileClock, HandCoins, Percent, SendHorizonal, UserPlus, WalletCards } from "lucide-react";
...
import { crearSolicitudPrestamo } from "../lib/solicitudService";
...
function getCalculation(form: RequestForm): FixedLoanCalculation | null {
...
export function NewLoanRequestPage() {
  ...
  function startAnother() {
    ...
    setForm({ ...INITIAL_FORM, fechaInicio: hondurasToday(), diaPagoSemana: getDefaultWeeklyPaymentDay(hondurasToday()) });
    setSent(false);
    setError("");
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : "No pudimos enviar la solicitud. Revise la conexión e intente de nuevo.");
    } finally {
      setSaving(false);
    }
  }
  ...
  if (sent) {
    return (
      <div className="space-y-4 pf-safe-page">
        <PageHero title="Solicitar préstamo" />
        <Card className="mx-auto max-w-lg space-y-4 p-6 text-center">
          <CheckCircle2 className="mx-auto h-12 w-12 text-pf-success" strokeWidth={2} aria-hidden />
          <p className="text-lg font-extrabold text-pf-text">Solicitud enviada</p>
          <p className="text-sm text-pf-muted">
            {selectedCustomer?.nombre ? <>La solicitud para <strong>{selectedCustomer.nombre}</strong> quedó</> : "La solicitud quedó"} pendiente
            de revisión. La cuenta maestra la aprobará o la rechazará; le avisaremos el resultado.
          </p>
          <div className="flex flex-col-reverse gap-2 pt-2 sm:flex-row sm:justify-center">
            <Button type="button" variant="secondary" onClick={() => navigate("/prestamos/mis-solicitudes")}>
              <FileClock className="h-4 w-4" strokeWidth={2} aria-hidden />
              Ver mis solicitudes
            </Button>
            <Button type="button" onClick={startAnother}>Solicitar otro préstamo</Button>
          </div>
        </Card>
      </div>
    );
  ...
  return (
    <form className="space-y-4 pf-safe-page max-md:pb-24" onSubmit={(event) => void submit(event)}>
      ...
      >
        <p className="pf-page-lead">Complete los datos del préstamo; la cuenta maestra decide si se aprueba.</p>
        <p className="pf-page-lead-muted">Esta solicitud no crea el préstamo todavía.</p>
      </PageHero>
      ...
      {customersError ? (
        ...
        <Card>
          <EmptyState
            ...
            description="Toda solicitud de préstamo debe quedar asociada a una persona con sus datos de contacto."
            action={
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(290px,1fr)] lg:items-start">
          <Card className="space-y-5 p-4 sm:p-5 lg:col-start-1 lg:row-start-1">
            ...
            <Field label="Cliente *" htmlFor="request-customer">
              <Select
                id="request-customer"
                data-autofocus="true"
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Monto a prestar (L) *" htmlFor="request-capital">
                <Input
                  id="request-capital"
            ...
            {form.frecuencia === "diario" ? (
              ...
              <Field label="Interés fijo total (%) *" htmlFor="request-rate">
                <Input
                  id="request-rate"
                  className={NUMBER_INPUT_CLASS}
                  type="number"
            ...
            <Field label="Fecha de inicio / desembolso *" htmlFor="request-start">
              <Input
                id="request-start"
                type="date"
            ...
            <Field label="Observaciones" htmlFor="request-notes">
              <Textarea
                id="request-notes"
                value={form.observaciones}
                onChange={(event) => setForm((current) => ({ ...current, observaciones: event.target.value }))}
                placeholder="Motivo del préstamo, referencias adicionales, etc. (opcional)"
                rows={3}
              />
            </Field>
            ...
            <div className="rounded-xl border border-pf-info-soft bg-pf-info-soft/35 p-3 text-xs leading-relaxed text-pf-text-secondary">
              <p className="flex items-start gap-2">
                ...
                <span>
                  ...
                  {calculation ? <> El primer pago vencería el <strong>{formatDateOnly(calculation.fechaPrimerPago)}</strong>.</> : null}
                </span>
              </p>
            </div>
          </Card>
          ...
          <Card className="space-y-4 p-4 sm:p-5 lg:sticky lg:top-[11.5rem] lg:col-start-2 lg:row-start-1">
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-pf-muted">Resumen de la solicitud</p>
            ...
            <Button type="submit" className="hidden min-h-[52px] w-full text-base shadow-lg md:inline-flex" disabled={saving || !calculation}>
              <SendHorizonal className="h-5 w-5" strokeWidth={2} aria-hidden />
          ...
          <Card className="space-y-4 p-4 sm:p-5 lg:col-start-1 lg:row-start-2">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-pf-muted">Se generarán si la cuenta maestra aprueba la solicitud.</p>
              </div>
            </div>
            {calculation ? (
              <InstallmentSchedule
